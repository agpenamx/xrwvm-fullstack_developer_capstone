...
def get_dealerships(request, state="All"):
    ...
    If a state is provided, filters dealerships by that state.
    Returns a JSON response.
    """
    endpoint = "/fetchDealers" if state == "All" else f"/fetchDealers/{state}"
    dealerships = get_request(endpoint)

    logger.debug(f"Fetching dealers for state: {state} -> Response: {dealerships}")  # ✅ Added logging
...
def get_dealer_details(request, dealer_id):
    try:
        ...
        logger.debug(f"Fetching dealer details for ID: {dealer_id} -> Response: {dealership}")  # ✅ Added logging
...
def get_dealer_reviews(request, dealer_id):
    try:
        ...
        logger.debug(f"Fetching dealer reviews for ID: {dealer_id} -> Response: {reviews}")  # ✅ Added logging
...
def add_review(request):
    ...
    try:
        ...
        logger.debug(f"Submitting review: {data} -> Response: {response}")  # ✅ Added logging
